...
코드 구현 완료 (직접 건드릴 필요 없음)
  ├── app/storage/db.py          SQLAlchemy async 엔진 + 세션 설정 (AI DB)
  ├── app/storage/rds.py         BE RDS 직접 쓰기 헬퍼 (save_transcript_draft, update_analysis_job_status)
  └── app/rag/vector_store.py    RagChunkORM 테이블 정의 + upsert / search

직접 준비해야 하는 것
  ├── [AI DB] PostgreSQL + pgvector 인스턴스 실행  (로컬: Docker / 운영: AWS RDS)
  ├── [AI DB] pgvector 확장 활성화                  CREATE EXTENSION vector
  ├── [AI DB] 테이블 생성                            scripts/init_db.sql 또는 create_tables.py 실행
  └── [BE RDS] 연결 정보 환경 변수 설정              BE_DATABASE_URL (ML GPU Worker에서만 사용)
```
...
```
app/storage/db.py
  └─ engine, Base 정의 (SQLAlchemy async 엔진 — AI DB / pgvector)

app/storage/rds.py
  ├─ save_transcript_draft()          ← transcripts + transcript_segments INSERT (BE RDS)
  └─ update_analysis_job_status()     ← analysis_jobs UPDATE (BE RDS)
...
`app/storage/rds.py`는 BE RDS에 직접 쓰는 헬퍼입니다. ML GPU Worker가 alignment 완료 후 호출하며, `BE_DATABASE_URL` 환경 변수로 연결합니다.

```python
from app.storage.rds import save_transcript_draft, update_analysis_job_status

# ML GPU Worker 내부
await save_transcript_draft(db, job_id, session_id, audio_file_id, draft_key, utterances)
