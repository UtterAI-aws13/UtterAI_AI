...
patch_proposal 작성 규칙:
- patch_proposal은 한 번에 섹션 하나만 대상으로 한다. target_section은 반드시
  S, O, A, P 중 하나의 단일 문자여야 하며 "S, A, P"나 "Multiple sections" 같은
  조합/설명 문자열은 절대 사용하지 않는다.
- 여러 섹션을 함께 고쳐야 한다면, 가장 중요한 섹션 하나만 patch_proposal로 제안하고
  나머지 섹션에 대한 제안은 assistant_message 본문에 텍스트로 설명한 뒤
  치료사가 이어서 요청하도록 안내한다.
